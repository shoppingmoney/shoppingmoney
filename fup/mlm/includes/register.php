...
function registerme($column, $value, $email ,$phone, $update=0){
    $conf = new config();
    $chk = $conf->checkAvailablity("firstimeregd where uemail='$email' OR ucontact='$phone'");
    if ($chk == 0 || empty($chk)) {
        ...
        echo "<script>alert('This email/mobile is already registered!!');</script>";
    }
    return $val;
}
