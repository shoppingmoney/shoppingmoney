...
$prc=$qty*$prct;
$crl=$dbAccess->countDtata("select * from buynow where product_id = '".$_GET['pid']."' And system_id = '$system_id'");
if($crl <= 0){
$dbAccess->insertData("buynow","product_id,unit_prc,mrp,shipping,point,qty,system_id,user_id","'".$_GET['pid']."','$prct','$prc','$shiponthedock','$mypointis','$qty','$system_id','$user_session_id'");
}else{
$dbAccess->updateData("update buynow set mrp=mrp+'$prc' , qty = qty+'$qty' where product_id = '".$_GET['pid']."' And system_id = '$system_id' And user_id = '$user_session_id' ");
}
echo "<script>alert('Item has add to your buynow');</script>";
...
<br/>-->
								 ...
								 ?>
									...
									<span class="text-stock">Weight : <?=$ship['weight'];?></span></p>
                                 </div>
                              </span>
                              </div>
...
<!-------clor picker--------->
                             <!--<div class="product-select product-Size" id="color_style">
                              ...
                              <input type="number" id="mytexy" value="1" min="1" max="<?=$fetch['commited_qty'];?>" name="qty" /></br></br></br>
                             <button type="submit" name="submit" class="toch-button toch-add-buynow">Add to buynow</button>
							 <a href="javascript:void(0)" class="toch-button toch-add-buynow" onclick="myFunction()" style="padding:11px;">Buy Now</a>
                              	<?php
									if(isset($_SESSION['myemail'])){
...
<i class="fa fa-heart-o wishlist" id="hard<?=$_GET['pid'];?>"></i></a>
									...
									<a href="#" class="modal-view toch-button toch-add-buynow" data-toggle="modal" data-target="#productModal2"  style="padding: 10px 1px;"
