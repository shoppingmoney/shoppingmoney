...
if(isset($_GET['delid'])){
	$dbAccess->updateData("delete from buynow where id = '".$_GET['delid']."' ");
...
<?php
								if(isset($_POST['btnsave'])){
									if($_POST['qty'] <= 0){
									  ...
									  echo "<script>alert('buynow has been updated successfully');</script>";
									  echo "<script>window.location='checkout.php';</script>";
									}else{
									$newprc = $_POST['oldprc']*$_POST['qty'];
									//echo "update buynow set qty = '".$_POST['qty']."',mrp='$newprc' where id = '".$_POST['oid']."' ";
									$dbAccess->updateData("update buynow set qty = '".$_POST['qty']."',mrp='$newprc' where id = '".$_POST['oid']."' ");
									echo "<script>alert('buynow has been updated successfully');</script>";
									echo "<script>window.location='checkout.php';</script>";
									}
...
$(function(){
 ...
 $("#did").css({"pointer-events": "none", "cursor": "default"}); 
	//$("#did").children().prop('disabled',true);
									...
									?>
													<a class="accordion-trigger <?=$cls?>" data-toggle="collapse" data-parent="#accordion" href="#checkout-confirm">2.Confirm Order<i class="fa fa-caret-down"></i> </a>
												</h4>
											</div>
...
<td width="10%"><p>Vat</p></td>
								...
								if($_SESSION['myemail']==null || $_SESSION['myemail']==''){
								$fetch = $prog->selectData("select * from buynow where system_id = '$system_id' And user_id = '' ");
								}else{
								$fetch = $prog->selectData("select * from buynow where user_id = '".$_SESSION['myemail']."'");	
								}
								$sum = 0;
$point = 0;
								foreach($fetch as $tr){
...
window.location="checkout.php?reevalue=yes&";
													...
													<div class="buttons pull-right">
														<a class="accordion-trigger collapsed btn btn-warning" data-toggle="collapse" data-parent="#accordion" href="#shipping-address">Confirm Order</a>
													</div>
			   ...
			   <div class="panel-heading">
			<h4 class="panel-title">
		<a id="did" class="accordion-trigger collapsed" data-toggle="collapse" data-parent="#accordion" href="#shipping-address">3.Delivery Details <i class="fa fa-caret-down"></i> </a>
		</h4>
		</div>
		<?php
		...
		?>
												...
												<form method="post" action="payment-now.php" id="undertaking">
...
$('#undertaking').attr('action', 'payment-now.php');
...
$('#undertaking').attr('action', 'payument-now.php');
//document.getElementsByTagName("undertaking").setAttribute("action", "payument-now.php");
document.getElementById("undertaking").submit();
}
</script>
    <section id="section3">
...
@media (min-width: 1366px) {
  ...
  .tabordion {
    width: 70%;
								...
								<?php
									if($_SESSION['myemail']!=null || $_SESSION['myemail'] != ''){
										$buynow_div = "display:block";
									}else{
										$buynow_div = "display:none";
									}
									?>
								
							<div class="col-xs-2" id="buynow_summary" Style="background-color:#FFFFFF;border: 1px solid #d0d0d0; padding:5px;<?=$buynow_div;?>">
							<?php
							$itme=0;
							if($_SESSION['myemail']==null || $_SESSION['myemail']==''){
								$fetch = $prog->selectData("select * from buynow where system_id = '$system_id' And user_id = '' ");
							...
							?>
							<h4 style="color: #26acce;font-weight:600;text-align: center;font-size:14px;">SUMMARY (<?php echo $item;?> Item)</h4>
...
//echo $tr['qty'];						
								...
								?>
								<table style="height:55px; width:100%;font-weight: 500;color:#000;">
							<tr><?=ucfirst($protitle['title']);?>)</tr>
							<tr style="background: #f5f5f5; padding-bottom:10px;" ><td width="58%">Quantity:<?=$tr['qty'];?></td>
...
<tr style="border-bottom: 1px solid black;padding-bottom:10px;"><td>Delivery Charges:</td>
						...
						<!--	<div id="buynowSummary" class="" style="max-height: 563px; position: relative;">
							<div id="buynow-summary"><div class=""><div class="buynow-heading">SUMMARY (1 Item)</div>
