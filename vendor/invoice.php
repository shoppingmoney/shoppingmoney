...
if(isset($_GET['orderid'])){
		...
		header("location:order_toUser.php");
		die();
}
                      ...
                      <div class="col-xs-12 table">
                        <table class="table table-striped">
						  ...
						  $tship = 0;
														...
														$prd=$dbAccess->selectData("SELECT * FROM checkout_ord WHERE orderid='".$oid."' AND vendor_id='".$vid."' AND billing_type='Customer Name' AND status > 2 AND status <> 9 ");
														foreach($prd as $pr){
                    ...
                    <div class="row no-print">
                      <div class="col-xs-12">
                       <!-- <button class="btn btn-default" onclick="window.print();"><i class="fa fa-print"></i> Print</button>
                        ...
                        <a href="invoice.php?orderid=<?=$oid?>&vid=<?=$vid?>&amount=<?=$total?>&landing=<?=$lsum?>" onclick="return confirm('Are you sure to generate this bill');" class="btn btn-primary pull-right" style="margin-right: 5px;"><i class="fa fa-download"></i> Generate Billing</a>
                      </div>
                    </div>
                  </section>
                </div>
