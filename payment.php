...
if(isset($_SESSION['cart'])){
	foreach ($_SESSION['cart'] as $value) {
		$grandmoney+= $value['quantity']*$value['price'];
	}
	$abc = $checkout->_addToCart($_SESSION['myemail'], $_SESSION['cart']);
	if($abc=="addedtocart"){
		unset($_SESSION['cart']);
	}else{
		echo "Some issue error occured";
	}
}else{
...
$state="";
/*End  Extra Code By Meraj*/
$userid=$dbAccess->selectSingleStmt("select * from firstimeregd where uemail = '".$_SESSION['myemail']."'");
$useid = $userid['id'];
$email = $_SESSION['myemail'];
// var_dump($checkout->connection->insert("checkout","orderid,user_email,fname,address,city,pincode,country,phone,date,status,state,user_id,point,grandtotla,mode","'$ordId','$email','$firstname','$address','$city','$postcode','$country','$phone',now(),'0','$state','$useid','$point','$pays','$actionon'"));
// die();
...
$dbAccess->insertData("checkout","orderid, user_email, user_id,  fname, lname, address, city, pincode, country,state, phone, date, mode, grandtotla, status","'$ordId','$email','$useid','$firstname','','$address','$city','$postcode','$country', '$state','$phone',now(),'WALLET','$grandmoney', '0'");

$Lastid = $dbAccess->LastId();

$totaamy = $dbAccess->selectSingleStmt("select sum(mrp) as coil from cart where user_id = '".$_SESSION['myemail']."' ");
$dbAccess->updateData("insert into account(uid,dr,cr,mode,date,status) values('$useid','".$grandmoney."','0','Shopping','".date('Y-m-d')."','1') ");
...
// $checkout->connection->insert("account", "uid, dr, cr, narration, mode, date, status", "{$email}, {$useid}, {$grandmoney}, {}, '', 'Shopping', '', '1' ");
...
// }
$dbAccess->insertData("checkout_ord","product_id,mrp,mrp_total,shipping,point,billing_type,qty,orderid,checkout_id,user_id,vendor_id,date,status","'".$t['product_id']."','".$t['unit_prc']."','".$t['mrp']."','".$t['shipping']."','".$t['point']."','".$dnatitoo['billing_type']."','".$t['qty']."','$ordId','$Lastid','$useid','".$zbon['vendor_id']."',now(),'0'");
}
...
<li style="list-style-type:none;font-size: 11px;"><b>Shipping Address</b></li>
<li style="list-style-type:none;font-size: 11px;">'.$_POST['firstname'].'<br>
...
<td>Title</td>
<td>Qty</td>
<td>Price</td><td>Shipping Chargs</td><td>Total</td></tr><tbody> ';
...
$mob_msg = array($phone,$copycat['phone'],'9554388551');
$coc=1;
foreach($mob_msg as $request_aai_hai){
if($coc==1){
...
$msg1 = urlencode("New order:- $ordId, notification amounting to:- Rs.$grandmoney, order by:- $firstname"); // otp sending code put here
}
$ID = 'argecom';
$Pwd = 'argecom';
$PhNo = $request_aai_hai; 
$Text = $msg1;
//mob_vendor 
...
$ab = $dbAccess->updateData("delete from cart where user_id = '".$_SESSION['myemail']."'");
if($ab){
	echo "Meraj Ahmad Siddiqui";
//echo  "<script>alert('purchased successfully');</script>";
//echo  "<script>window.location='index.php';</script>";
}else{
echo  "<script>window.location='mlm/panel.php?dash=order';</script>";
}
?>