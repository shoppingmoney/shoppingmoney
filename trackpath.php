			...
			<div class="container">
				...
				<div class="col-md-11" style=" padding-top:13px; padding-left: 0px;padding-right: 0px; margin-left:3%; background: #fff">
				     <div class="toch-review-title">
									...
									<?php
									$bts=$product=$dbAccess->selectSingleStmt("select * from checkout where orderid='".$_GET['orderid']."' ");
									?>
                                    <div class="col-md-12" id="updtl">
					<div class="col-md-10"><span>Order ID: <?=$_GET['orderid'];?></span><br>
					<span class="dti">Placed on <?=$bts['date'];?></span>
				    </div>				
...
<?php
//start here
$vizar=$dbAccess->selectData("select * from checkout_ord where orderid='".$_GET['orderid']."' ");
foreach($vizar as $viz){ ?>
		 
                     <div class="col-md-2">
                        <?php
						$name=$dbAccess->selectSingleStmt("select * from product_tbl where product_id = '".$viz['product_id']."'");
						$img=$dbAccess->selectSingleStmt("select * from product_img where product_id = '".$viz['product_id']."'");
						?>
                        <img src="<?=$img['image'];?>">
                     </div>
			
		<div class="col-md-7">
		<span class="prd"><p><?=$name['title'];?></p></p> </span>
	</div>	

    <div class="col-md-3" id="ird">
		<span class="badge1"><span class="ordrt">Order Total:</span> &nbsp; <span style="font-size: 15px;font-weight:600;">Rs <?php
				echo $viz['mrp']*$viz['qty'];
		?></span></span>
	</div>
	...
	<div class="col-md-12" style="background:#fff;">
		<div class="col-md-10" style="color:<?=$stcolor;?>">
			...
			}else if($viz['status']==1){
				echo "Order Has Been Recieved";
			}else if($viz['status']==2){
				echo "Order Has Been Packed And Ready To Dispatched";
			}else if($viz['status']==3){
				echo "Order Has Been Dispatched";
			}else if($viz['status']==4){
